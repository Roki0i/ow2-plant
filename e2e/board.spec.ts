...
test('rejects invalid local files without losing placements', async ({ page }) => {
  ...
  await page.locator('.image-settings input[type=file]').setInputFiles({ name: 'bad.svg', mimeType: 'image/svg+xml', buffer: Buffer.from('<svg/>') });
  await expect(page.getByRole('alert')).toContainText('PNG・JPEG・WebP');
  await expect(page.getByTestId('board').locator('canvas')).toBeVisible();
  await expect(page.getByTestId('element-count')).toHaveText('1 個配置');
  page.on('dialog', dialog => dialog.accept());
  await page.locator('.image-settings input[type=file]').setInputFiles({ name: 'broken.png', mimeType: 'image/png', buffer: Buffer.from('not a png') });
...
test('loads a local image without uploading it and resets incompatible placements', async ({ page }) => {
  ...
  await page.locator('.image-settings input[type=file]').setInputFiles(file);
  await expect(page.getByTestId('element-count')).toHaveText('1 個配置');
  page.once('dialog', dialog => dialog.accept());
  await page.locator('.image-settings input[type=file]').setInputFiles(file);
  await expect(page.locator('.map-notice').filter({ hasText: 'own-map.png' })).toBeVisible();
